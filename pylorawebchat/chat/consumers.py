    ...
    async def chat_message(self, event):
        if "node_pk" in event["message"].keys():
            ...
            message_model: Message = Message(
                node=node, message=message, message_type="o", instant_send=False
            )

            message_model.save()

            self.redis_con.rpush(
                "message_queue",
                MessageObject(address=node.address, msg=message).to_string(),
            )

            # Send message to WebSocket
            await self.send(
                text_data=json.dumps({"message": message_model.to_json(created=True)})
            )
        else:
            # Send message to WebSocket
